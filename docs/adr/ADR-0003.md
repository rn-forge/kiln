...
- `.rn-forge/kiln/state.json` is generated and committed, and `kiln check`
...
- Repeated setup steps live in a committed composite action, not in four copies
  inside one workflow.
- `kiln doctor` additionally renders current templates, to answer whether the
  pinned kiln or an edited config *would* change an artifact. It is heavier,
  so CI runs it only where the workflow asks. `kiln apply` refuses unapproved
  drift.
...
`kiln check` runs over both baselines rather than being forked.
