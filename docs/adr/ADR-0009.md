...
*Accepted 2026-09-15; see [History](#history).*
...
[F3.4](../specs/epics/E3-realign-goldens-and-canon/F3.4-canon-catches-up.md).
The acceptance test above holds: E2's F2.10 recorded it as met (D49), and F3.1
re-proved it on pykit's Part E API — both CLI goldens are
`app = CliApp.from_config(CONFIG)` with no `main()`.

### Proposed, then revised after the Phase C review

Proposed rather than accepted because `rn-forge-cli` and `rn-forge-tooling` did
not exist yet: they were extracted in Phase C, and this ADR was what that
extraction aimed at. The Phase C review then split the boilerplate across two
packages — the command-line boilerplate is `rn-forge-cli`, the file-owning
machinery `rn-forge-tooling` ([ADR-0002](ADR-0002.md) says why the seam moved).
