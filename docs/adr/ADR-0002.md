...
[ADR-0009](ADR-0009.md) says a repo should never write. It does not want the
generation engine, the template engine, the local state store or the installer.
Under one package it had to take all of them, from a distribution whose own
contract told deployed code not to depend on it — so it wrote its own `main()`
instead, which is the outcome ADR-0009 exists to prevent.
...
| **developer tool** | `rn-forge-tooling` | the generation engine, the template engine, local state, installer mechanics | commons, cli, jinja2 |
...
([ADR-0001](ADR-0001.md)). pykit adopting kiln as dev tooling is not a cycle,
...
- **At the dependency**, by `scripts/standards/check_rn_forge_deps.py`: which
  ...
  ([ADR-0005](ADR-0005.md)). import-linter rejects subpackages of external
  packages, so `rn_forge.kiln` cannot be named as a forbidden module —
...
- "Which package does this belong in?" has a mechanical answer read off the
  ...
  A capability every kiln-generated repo needs and nothing else uses — the
  docs structure, nav and site checks — is kiln's too, in the module that owns
  the concern.
...
### 2026-09-16 — docs mechanics move from tooling into kiln's `docs` module

The developer-tool row listed "docs mechanics", and `rn-forge-tooling` shipped
them with an `rn-forge-docs` console script. The owner questioned it: tooling is
the machinery behind installable tools, whereas the docs area model, nav
generation and site checks are a capability every kiln-generated repo needs and
no other consumer uses. That makes them rn-forge repository policy by this ADR's
own test, so they moved into kiln's `docs` module, and `rn-forge-docs` became
`kiln doctor --only docs-structure|docs-nav|docs-site` and `kiln docs-nav`
