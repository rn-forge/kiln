...
# The repository standard — kiln

This is `docs/reference/standard-repo.md` rendered with this repo's archetype,
docs profile and task surface filled in — kiln's own copy of the standard it
renders into every other repo. It is the normative
...
files (kiln ADR-0003). Repository validation tasks may check these independently.
Only managed files and fenced blocks remain kiln's responsibility.

The current checker implementation still includes checks over unowned content.
Kiln's F4.6 ownership-alignment story tracks their removal; this is the target
contract, not a claim that those checks have already been removed.
...
before the change is done (kiln render-matrix specification).
...
rn-forge packages this repo may import. Every other rn-forge component is a
subprocess or nothing.

## 5. What CI checks, and what it does not

CI runs the pinned kiln read-only (kiln ADR-0006): it never runs `kiln apply`
...
modules under one contract (kiln ADR-0001) — each owns its config section, its
