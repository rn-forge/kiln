...
For v1 the shipped web cells are `python-web-app` with **`fastapi + angular`**
and `python-web-api` with **`fastapi`**. `django`, `react`, and `svelte` are
named and `untested`; `kiln new` refuses an untested value without an explicit
override. Django is promoted separately when a real repository needs it and can
prove its scaffold.
...
- 2026-09-17: defer Django for both web archetypes and make FastAPI the
  `python-web-app` default; the first real repositories in scope use FastAPI,
  while Django still needs its framework-owned scaffold.
