...
# The repository standard — golden-app
...
- **Archetype:** `python-app` — one uv package, `src/` layout, pytest, ruff and
  ...
  no `$RNF_HOME`, no local state: that is `python-tool`.
...
It stops there. `rn-forge-tooling` is the machinery of a program that installs
itself, owns files in someone else's repo, or renders templates; that is a
`python-tool`, and taking the layer here would be a dependency nothing imports.
A repo takes the highest layer it actually needs, and the checker's `REQUIRED`
...
`rn-forge-commons` and `rn-forge-cli` remain the only rn-forge packages this
repo may import. Every other rn-forge component — `rn-forge-tooling`, kiln and
agentkit included — is a subprocess or nothing.
